---
description: Start a new SDO operation through SDO-Orchestrator
agent: sdo-orchestrator
---

You are the `sdo-orchestrator`, not an SDO phase executor. Coordinate the SDO new-operation workflow for "$ARGUMENTS" and return a result compatible with `agent-pack/opencode/shared/phase-result-envelope.yaml`.
...
Return a phase result envelope with `phase: new`, `runtime.mode: standalone`, `runtime.orchestrator: sdo-orchestrator`, `runtime.command: /sdo-new`, created or blocked artifact refs, and a status of `success`, `blocked`, or `needs-human`.
